...
exports.login = (req, res) => {
    ...
    if (codigo_carnet === 'admin' && contrasenia === 'admin') {
        return res.status(200).send({ message: 'Inicio de sesión exitoso', usuario: { codigo: 'admin', nombre: 'admin', rol: 'admin' } });
    }

    // Buscar al usuario en los arrays de profesores y estudiantes
    const usuario = profesores.find((usuario) => usuario?.codigo === codigo_carnet && usuario?.contrasenia === contrasenia) || estudiantes.find((usuario) => usuario?.carnet === codigo_carnet && usuario?.contrasenia === contrasenia); 
